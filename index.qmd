...
![](assets/tension.png){fig-align="center" width="66%"}
...
::: {#fig-energy-ridgeplot layout-ncol=2}
...
- Use these to update $\Gamma^{\pm}: (x, v) \rightarrow \left(x, v_{\pm}\right)$[^inverse]:
    ...
    - [forward]{style="color:#FF5252"} $(d = \textcolor{#FF5252}{+})$:
      $$v_{\textcolor{#FF5252}{+}} = v \cdot e^{\frac{\varepsilon}{2} s_{v}} - \frac{\varepsilon}{2}\left[ F \cdot e^{\varepsilon q_{v}} + t_{v} \right]$$


    - [backward]{style="color:#1A8FFF;"} $(d = \textcolor{#1A8FFF}{-})$: 
      $$v_{\textcolor{#1A8FFF}{-}} = e^{-\frac{\varepsilon}{2} s_{v}} \left\{v + \frac{\varepsilon}{2}\left[ F \cdot e^{\varepsilon q_{v}} + t_{v} \right]\right\}$$


[^constants]: $\lambda_{s},\, \lambda_{q} \in \mathbb{R}$ are trainable parameters
[^inverse]: Note that $\left(\Gamma^{+}\right)^{-1} = \Gamma^{-}$
