...
::: {style="font-size:0.75em; text-align: left; margin-left: inherit;"}
> Obviously, an independent cross-check of the BMW lattice result for
> $a_{\mu}^{\mathrm{hvp, LO}}$ with sub-percent precision is badly needed  
> --- [@wittig2023progress]
:::
...
> **Note**: $\partial_{x} S(x)$ is the _force_
:::
...
<iframe data-src="https://chi-feng.github.io/mcmc-demo/app.html" width="100%" height="650" title="l2hmc-qcd"></iframe>
...
# L2HMC Update {style="font-size:0.8em;"}
...
$$\begin{align*}
S_{\beta}(x) &= \beta\sum_{P} \cos \textcolor{#00CCFF}{x_{P}}, \text{ where } \\
...
> $1\times 1$ square, called a ["plaquette"]{.blue-text}
:::

:::

::: {.column width="50%"}
![](https://raw.githubusercontent.com/saforem2/deep-fridays/main/assets/u1lattice.dark.svg){width="100%"}
:::
...
![Critical Slowing Down at increasing $\beta$](https://raw.githubusercontent.com/saforem2/l2hmc-dwq25/main/docs/assets/critical_slowing_down.svg){width="80%"}
...
- Want to maximize the _expected_ squared charge difference[^charge-diff]:
  $$\begin{equation*}
  \mathcal{L}_{\theta}\left(\xi^{\ast}, \xi\right) = {\mathbb{E}_{p(\xi)}}\big[-\textcolor{#FA5252}{{\delta Q}}^{2} \left(\xi^{\ast}, \xi \right)\cdot A(\xi^{\ast}|\xi)\big]
  \end{equation*}$$
...
- Where:
    ...
    - $A(\xi^{\ast}|\xi)$ is the probability of accepting the proposal $\xi^{\ast}$:
      $$\begin{equation*}
      A(\xi^{\ast}|\xi) = \mathrm{min}\left( 1, \frac{p(\xi^{\ast})}{p(\xi)}\left|\frac{\partial \xi^{\ast}}{\partial \xi^{T}}\right|\right)
      \end{equation*}$$

[^charge-diff]: Here, $\xi^{\ast}$ is the _proposed_ configuration (prior to Accept / Reject)
...
by comparing $\tau_{\mathrm{int}}^{Q}$ for the  
...
![Eval](https://saforem2.github.io/anl-job-talk/assets/dQint_eval.svg){#fig-eval}

![HMC](https://saforem2.github.io/anl-job-talk/assets/dQint_hmc.svg){#fig-hmc}

Comparison of $\delta Q$ for the trained model [@fig-eval] vs. HMC [@fig-hmc]
:::
...
# 4D $SU(3)$ Model {.centeredslide auto-animate="true"}
...
<iframe data-src="https://nbviewer.org/github/saforem2/l2hmc-qcd/blob/dev/src/l2hmc/notebooks/l2hmc-2dU1.ipynb" width="100%" height="650" title="l2hmc-qcd"></iframe>
