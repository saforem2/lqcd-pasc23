...
[^hmc]: [Hybrid Monte Carlo, Duane et. al Phys.Lett.B 195 (1987)](https://www.sciencedirect.com/science/article/pii/037026938791197X?via=ihub)
...
::: {.column width="50%" style="text-align: center; margin-left: 1%;"}

::: {.callout-important title="Hamiltonian Dynamics" style="font-size:1.1em;"}
$\left(\dot{x}, \dot{v}\right) = \left(\partial_{v} H, -\partial_{x} H\right)$
:::

::: {.callout-note title="Leapfrog Step" style="font-size:1.1em;"}
...
$v_{1} := \textcolor{#EC407A}{\Gamma}(x_{0}; v_{0}) = v_{0} - \frac{\varepsilon}{2} \partial_{x} S(x_{0})\hspace{3px}$

<!-- 2. [update $\textcolor{#D0BFFF}{x}$]{style="font-family:monospace"} -->
$x' := \textcolor{#FD971F}{\Lambda}(x_{0}; v_{1}) = x_{0} + \varepsilon \, v_{1}\hspace{29px}$

<!-- 3. [update $\textcolor{#B2F2BB}{v}$]{style="font-family:monospace"} -->
$v' := \textcolor{#EC407A}{\Gamma}(x'; v_{1}) = v_{1} - \frac{\varepsilon}{2} \partial_{x} S(x')\hspace{8px}$ 

:::

::: {.callout-warning title="Warning!"}
$v_{0} \sim \mathcal{N}(0, \mathbb{1})$ at the [beginning]{.yellow-text} of each trajectory
:::

:::

::: {.column width="49%"}
![](./assets/hmc/hmc-update-dark.svg){width=66%}
:::
...
$$\begin{align*}
  \textcolor{#ec407a}{\Gamma}: (x, v) \textcolor{#ec407a}{\rightarrow} v' &:= v - \frac{\varepsilon}{2} \partial_{x} S(x) \\
...
![](https://raw.githubusercontent.com/saforem2/l2hmc-dwq25/main/docs/assets/drawio/network_functions.svg){width="90%"}
...
6. Evaluate MH criteria and assign the next state in the chain according to
  ...
  \mathbf{x}_{i+1}\gets
  \begin{cases}
    \textcolor{#AE81FF}{\mathbf{x}^{\ast}} \small{\text{ w/ prob }} A(\textcolor{#AE81FF}{\xi^{\ast}}|\textcolor{#00CCFF}{\xi}) \hspace{29pt}✅ \\
    \textcolor{#00CCFF}{\mathbf{x}} \hspace{14px}\small{\text{ w/ prob }} 1 - A(\textcolor{#AE81FF}{\xi^{\ast}}|{\textcolor{#00CCFF}{\xi}}) \hspace{10pt}❌
  \end{cases}
  $$
:::
...
:::
![](https://raw.githubusercontent.com/saforem2/deep-fridays/main/assets/u1lattice.dark.svg){width="45%"}
:::
